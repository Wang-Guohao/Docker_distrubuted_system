...
def login():
    ...
    if request.method == 'POST':
        ...
        db = dbconn.Database(host='db2', username='root', password='123456', database='datacenter')
        db.connect()
...
def praseToEvents()-> str:
    ...
    horror_sub = db.findAll(table='subscription', where='topicid = 1')
    horror_userids = [h['userid'] for h in horror_sub]
    action_sub = db.findAll(table='subscription', where='topicid = 2')
    action_userids = [a['userid'] for a in action_sub]
    drama_sub = db.findAll(table='subscription', where='topicid = 3')
    drama_userids = [d['userid'] for d in drama_sub]
    other_sub = db.findAll(table='subscription', where='topicid = 4')
    other_userids = [o['userid'] for o in other_sub]
    ...
    for event in res:
        eid = event['id']
        h_count = db.count(table='genre', where=f"tid='{eid}' and genre='{genre[0]}'")
        a_count = db.count(table='genre', where=f"tid='{eid}' and genre='{genre[1]}'")
        d_count = db.count(table='genre', where=f"tid='{eid}' and genre='{genre[2]}'")
        o_count = db.count(table='genre', where=f"tid='{eid}' and genre not in{tuple(genre)}")
        if h_count > 0:
            event['genre'] = genre[0]
            new_id = db.insert('event', {'content': str(event), 'topicid': 1})
            t1 = db.find(table='topic',where='topicid=1')
            if t1['advertise'] == 1:
                for u in user_ids:
                    db.insert(table='user_event', params={'userid':u, 'eventid':new_id})
            else:
                for h in horror_userids:
                    db.insert(table='user_event', params={'userid':h, 'eventid':new_id})
        elif a_count > 0:
            event['genre'] = genre[1]
            new_id = db.insert('event', {'content': str(event), 'topicid': 2})
            t2 = db.find(table='topic',where='topicid=2')
            if t2['advertise'] == 1:
                for u in user_ids:
                    db.insert(table='user_event', params={'userid':u, 'eventid':new_id})
            else:
                for a in action_userids:
                    db.insert(table='user_event', params={'userid':a, 'eventid':new_id})
        elif d_count > 0:
            event['genre'] = genre[2]
            new_id = db.insert('event', {'content': str(event), 'topicid': 3})
            t3 = db.find(table='topic',where='topicid=3')
            if t3['advertise'] == 1:
                for u in user_ids:
                    db.insert(table='user_event', params={'userid':u, 'eventid':new_id})
            else:
                for d in drama_userids:
                    db.insert(table='user_event', params={'userid':d, 'eventid':new_id})
        elif o_count > 0:
            event['genre'] = 'other'
            new_id = db.insert('event', {'content': str(event), 'topicid': 4})
            t4 = db.find(table='topic',where='topicid=4')
            if t4['advertise'] == 1:
                for u in user_ids:
                    db.insert(table='user_event', params={'userid':u, 'eventid':new_id})
            else:
                for o in other_userids:
                    db.insert(table='user_event', params={'userid':o, 'eventid':new_id})
    db.close()
    return {'msg':'success'}
...
def localSubscribe(userid, topic=None):
    ...
    for sub in subscriptions:
        if sub['topicid'] == topicid:
            db.close()
            return {'sublist': subscriptions}
    db.insert('subscription', {"userid": userid, "topicid": topicid, "name": ('gr'+'-'+topic)})
    subscriptions = db.findAll('subscription',"userid='%s'"%userid)
    db.close()
    return {'sublist': subscriptions}
...
def notify() -> str:
    ...
    if localTopicStatus or broker1.get('result') or broker3.get('result'):
...
@app.route('/getsubs/gr/<userid>', methods=['GET'])
def send_subdata2(userid):
    db = dbconn.Database(host='db', username='root', password='123456', database='datacenter')
    db.connect()
    output = db.findAll(table='subscription', where=f'userid = {userid}')
    db.close()
    print('gr subs: ' + json.dumps(output))
    return jsonify(output)

@app.route('/get_submovie/gr/<topic>', methods=['GET'])
def send_grsubdata(topic):
    db = dbconn.Database(host='db', username='root', password='123456', database='datacenter')
    db.connect()
    gr_mcodes = {}
    movies = {}
    output = db.findAll(table='genre', where=f"genre = '{topic}'")
# print("gr: " + json.dumps(output))
    for y in output:
        gr_mcodes[y['tid']] = 0
    for x in gr_mcodes:
        output = db.find(table='Movie', where=f"id = '{x}'")
        movies[output['id']] = output
    db.close()
